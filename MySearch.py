...
class MySearch:
	...
	def perform(self, problem):
		...
		queue = PriorityQueue()						# Setup the data structure for storing nodes to explore and put the root in it
		queue.push(root, 0)
		...
		while len(queue) > 0:
			...
			if problem.goalReached(state):
				...
				print 'Evaluation rate =', expanded/ellapsed, 'nodes/sec.'
			...
			for action in problem.actions(state):
				...
				if result not in found:
					...
					#queue.push(child, (problem.heuristic(result) + node.pathCost()))
					queue.push(child, problem.heuristic(result))
					found.add(result)
